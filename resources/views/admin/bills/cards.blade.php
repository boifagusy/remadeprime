        ...
        <table class="table table-bordered table-hover" id="datatable">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{10 ?? "Nill"}}</td>
                    <td><span class="badge @if($network->cardpin == 1)bg-success @else bg-danger @endif">@if($network->cardpin == 1)active @else disabled @endif </span></td>
                    <td>
                        <div class="dropstart">
                        ...
                        <div class="dropdown-menu">
                            ...
                            @if($network->cardpin == 1)                               
                            <a class="dropdown-item" href="{{route('admin.bills.cards.status' ,[$network->id, 0])}}">Disable</a> @else                              
                            <a class="dropdown-item" href="{{route('admin.bills.cards.status' ,[$network->id, 1])}}">Enable</a>  
                            @endif
                        </div>
                        </div>
                    </td>
                ...
                <div class="modal fade" id="edit_modal-{{$network->id}}" tabindex="-1"  aria-hidden="true">
                    <div class="modal-dialog">
                        ...
                        <div class="modal-body">
                            <form action="{{route('admin.bills.cards.update',$network->id)}}" enctype="multipart/form-data" method="post">
                                @csrf
                                <div class="form-group">
                                    <label class="form-label">Plan Code</label>
                                    <input type="text" class="form-control" name="p_code" placeholder="Plan Code" value="{{$network->p_code}}" required>
                                </div>
