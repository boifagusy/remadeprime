...
<div class="card">
    <div class="card-header">
        ...
        <a class="btn btn-primary btn-sm" href="#" data-bs-toggle="modal" data-bs-target="#createPlan"><i class="fas fa-plus"></i> New Plan</a>
        </div>
    </div>
        ...
        <table class="table table-bordered table-hover" id="datatable">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{$key +1}}</td>
                    <td>{{$plan->name}}</td>
                    <td><img loading="lazy"  class="img-table" src="{{my_asset($plan->decoder->image)}}" alt="{{$plan->decoder->name}}"> </td>
                    ...
                    <td>
                        ...
                        <div class="dropdown-menu">
                            <a class="dropdown-item" data-bs-toggle="modal" data-bs-target="#editPlan-{{$plan->id}}"  href="#" >@lang('Edit Plan')</a>
                            @if($plan->status == 1)                               
                            <a class="dropdown-item" href="{{route('admin.bills.cableplan.status' ,[$plan->id, 0])}}">Disable</a> @else                              
                            <a class="dropdown-item" href="{{route('admin.bills.cableplan.status' ,[$plan->id, 1])}}">Enable</a>  
                            @endif
                        </div>
                        </div>
                    </td>
...
<div class="modal fade" id="createPlan" tabindex="-1"  aria-hidden="true">
    <div class="modal-dialog modal-lg">
        ...
        <div class="modal-body">
            <form action="{{route('admin.bills.cabletv.create')}}" enctype="multipart/form-data" method="post">
                @csrf
                <input type="hidden" name="decoder_id" value="{{$decoder->id}}">
                <div class="form-group">
                    <label class="form-label">Decoder</label>
