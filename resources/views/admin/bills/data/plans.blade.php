        ...
        <table class="table table-bordered table-hover" id="datatable">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{$key +1}}</td>
                    <td>{{$plan->name}}</td>
                    <td><img loading="lazy"  class="img-table" src="{{my_asset($plan->network->image)}}" alt="{{$plan->network->name}}"> </td>
                    <td>{{$plan->service}}</td>
                    ...
                    <td>
                        ...
                        <div class="dropdown-menu">
                            ...
                            @if($plan->status == 1)                               
                            <a class="dropdown-item" href="{{route('admin.bills.dataplan.status' ,[$plan->id, 0])}}">Disable</a> @else                              
                            <a class="dropdown-item" href="{{route('admin.bills.dataplan.status' ,[$plan->id, 1])}}">Enable</a>  
                            @endif
                        </div>
                        </div>
                    </td>
...
<div class="modal fade" id="createPlan" tabindex="-1"  aria-hidden="true">
    <div class="modal-dialog modal-lg">
        ...
        <div class="modal-body">
            <form action="{{route('admin.bills.dataplan.create')}}" enctype="multipart/form-data" method="post">
                @csrf
                <input type="hidden" name="network_id" value="{{$network->id}}">
                <div class="form-group">
                    <label class="form-label">Network</label>
