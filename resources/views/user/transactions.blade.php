    ...
    font-size: 11px; /* Change this value as needed */
      ...
      <div class="centered-select">
            <div class="custom-select-container">
                ...
                <option value="internet">airtime2cash</option>
                <option value="internet">wallet</option>
                </select>
            </div>
        </div>
...
<a href="#" class="transaction-card" data-type="{{short_trx_type($item->service)}}" data-bs-toggle="modal" data-bs-target="#transactionModal">
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            <div>
                                <h5>{{short_trx_type($item->service)}}</h5>
                                <p class="transaction-date"> {{show_datetime($item->created_at)}}  </p>
                            </div>
                            <div class="text-end">
                                <div class="transaction-amount text-success">{{format_price($item->amount)}}</div>
                                <div class="transaction-status transaction-successful">
                                @if($item->status == 1)
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-6 transaction-details">{{$item->code}}</div>
                    </div>
                    <div class="separator"></div>

                    <div class="row">
                        <div class="col-6 transaction-details-label">Date:</div>
                        <div class="col-6 transaction-details">{{show_datetime($item->created_at)}}</div>
                    </div>
                    <div class="separator"></div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-6 transaction-details"> {{format_price($item->amount)}} </div>
                    </div>
                    <div class="separator"></div>

                    <div class="row">
                        <div class="col-6 transaction-details-label">Charge:</div>
                        <div class="col-6 transaction-details">{{format_price($item->charge)}}</div>
                    </div>
                    <div class="separator"></div>

                    <div class="row">
                        <div class="col-6 transaction-details-label">New Balance:</div>
                        <div class="col-6 transaction-details">{{format_price($item->new_balance)}}</div>
                    </div>
                    <div class="separator"></div>

                    <div class="row">
                        <div class="col-6 transaction-details-label">Type :</div>
                        <div class="col-6 transaction-details">{!! trans_type($item->type) !!} </div>
                    </div>
                    <div class="separator"></div>

                    <div class="row">
                        <div class="col-6 transaction-details-label">Details:</div>
                        <div class="col-6 transaction-details"> {{$item->message}} </div>
                    </div>
                </div>
    ...
    <script>
        ...
        document.getElementById('whatsappBtn').addEventListener('click', function () {
            ...
            html2canvas(document.getElementById('receiptContent')).then(function (canvas) {
                ...
                const customerCareNumber = '2349066962042'; // Replace with your actual WhatsApp number (without +)
